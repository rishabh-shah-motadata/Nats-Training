...
func main() {
	nc, err := nats.Connect(
		...
		nats.Name("NATS Tenant-1 Service Endpoint"),
	...
	group1 := service.AddGroup("tenant-1")
...
func getHandler(req micro.Request) {
	var response []byte
	log.Println("Received GET request for tenant-1")
	response = []byte("Response from tenant-1 GET endpoint")
	req.Respond(response)
}

func postHandler(req micro.Request) {
	var response []byte
	log.Println("Received POST request for tenant-1 with data:", string(req.Data()))
	response = []byte("Response from tenant-1 POST endpoint")
	req.Respond(response)
}
